...
export class NewsService {
  api =
    'https://newscatcher.p.rapidapi.com/v1/latest_headlines?topic=news&lang=en&media=true';
  searchApi =
    'https://newscatcher.p.rapidapi.com/v1/search_free?lang=en&media=true';
  httpHeaders = new HttpHeaders()
    .set('X-RapidAPI-Key', '5bd3090b1emsh4eb8caef68a502cp1a9a87jsn318b9253b740')
  ...
  getLatestNews(num: number): Observable<NewsTemp> {
    return this.http
      .get<NewsTemp>(this.api, { headers: this.httpHeaders })
      .pipe(map((data: any) => data.articles.slice(0, num)));
  }
  searchNews(num: number, keyword: string): Observable<NewsTemp> {
    let httpParams = new HttpParams().set('q', keyword);
    return this.http
      .get<NewsTemp>(this.searchApi, {
